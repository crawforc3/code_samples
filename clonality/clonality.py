...
def parse_arbitrary(metadata, arbitrary_list):
    """Rewrite list elements into dictionaries"""
    # arbitrary_list becomes a list of dictionaries in this function
    for arb_num, arb_value in enumerate(arbitrary_list):
        arbitrary_list[arb_num] = {}

    for key, value in metadata.items():
        # loops through each individual values (j = list element, v = value)
        for j, v in enumerate(value):
            # Add each value to its dictionary in arbitrary_list
            try:
                if value[j] is not '':
                    arbitrary_list[j][key] = value[j]
                else:
                    continue
            except IndexError:
                arbitrary_list = [{key: value[j]}]

    return arbitrary_list


def clean_data(parsed_data, fname_var):
    """Clean out None types from data"""
    if 'clonality' in fname_var or 'tcell' in fname_var:
        # In this case, parsed_data will be a dictionary
...
def prep_meta(meta_file_to_prep='metadata.tsv'):
    ...
    try:
        with open(meta_file_to_prep, 'r') as meta_file:
            ...
            if '' in header or '\n' in header:
                error_log(
                    "There is a missing header in the metadata file. Every column should have a header. "
                    "Column one is reserved for GROUPS, column two for SAMPLE FILES, and any additional "
                    "columns need Custom Names for headers.")
            ...
            for i, arb in enumerate(arblist):
                arblist[i] = arb.translate({ord(c): "" for c in r'/\:*?"<>|'})
            ...
            for line in meta_file:
                ...
                # When there is no arbitrary
                    ...
                    # Probably a blank row.
                        ...
                        error_log(
                            "Found blank cells in "
                            + meta_file_to_prep
                            + ". Please remove those rows entirely or fill in the information.")
                        continue
    ...
    groups = [g for (g, s) in iter(meta_dictionary.keys())]
    if len(set(groups)) < 2:
        error_log(
...
def config_checker(cfg_file):
    ...
    try:
        ...
        error_log(
            '"' + config_correction + '"' +
            ' is not a valid option for Correction. '
            'The defualt "None" was used. Multiple correction options are Bonferroni, BH, or None.')
    ...
    try:
        ...
        error_log(
            '"' + config_masterfile + '"' + ' is not a valid option for masterfile. '
                                            'The defualt "True" was used. Master file options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_output_verbose + '"' + ' is not a valid option for verbose. '
                                                'The defualt "False" was used. '
                                                'Verbose options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_error_log + '"' + ' is not a valid option for Error Log. '
                                           'The defualt "True" was used. Error Log options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_overwrite + '"' + ' is not a valid option for Overwrite. '
                                           'The defualt "True" was used. Options are True or False.')
    ...
    try:
        ...
        if config_order:
            config_dict['config_masterfile'] = False
            error_log(
                'Note: Since overwrite is set to True, '
                'Masterfile has automatically been set to False to prevent FileNotFound errors.')
    except AssertionError:
        config_dict['config_order'] = False
        error_log(
            '"' + config_order + '"' + ' is not a valid option for Custom Order. '
                                       'The defualt "False" was used. Custom Order options are True or False.')

    try:
        config_xrotation = config.get('graph_options', 'X-label rotation')
        config_xrotation = int(config_xrotation)
        assert -90 <= config_xrotation <= 90
        config_dict['config_xrotation'] = config_xrotation
    except AssertionError as error:
        config_dict['config_xrotation'] = 0
        error_log(str(error) + ' "' + str(
            config_xrotation) + '"' + ' is not a valid option for X-Label rotation. '
                                      'Value must be an integer between -90 and +90. The defualt 0 was used.')
    ...
    try:
        ...
        error_log(
            '"' + config_boxplots + '"' + ' is not a valid option for boxplots. '
                                          'The defualt "True" was used. Boxplot options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_boxcolors + '"' + ' is not a valid option for boxcolors. '
                                           'The defualt "True" was used. Boxplot color options are True or False.')
    ...
    if 'none' in config_boxpalette.lower():
        ...
        try:
            ...
            error_log(
                "Value Error: Couldn't convert "
                + config_boxpalette +
                " into a list of integers for the box plot color palette. "
                "Make sure you are using integers separated by a single comma e.g. 1,2,3,4,5,6 "
    ...
    try:
        ...
        error_log(
            '"' + config_stripplots + '"' + ' is not a valid option for stripplots. '
                                            'The defualt "True" was used. Strip plot options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_dotcolors + '"' + ' is not a valid option for dotcolors. '
                                           'The defualt "True" was used. Dot color options are True or False.')
    ...
    if 'none' in config_dotpalette.lower():
        ...
        try:
            ...
            error_log(
                "Value Error: Couldn't convert "
                + config_dotpalette + " into a list of integers for the dot color palette. "
                                      "Make sure you are using integers separated by a single comma e.g. 1,2,3,4,5,6 "
                                      "\nUsing default Adaptive colors' ")
            config_dict['config_dotpalette'] = None
    ...
    try:
        ...
        error_log(
            '"' + config_jitter + '"' + ' is not a valid option for jitter. '
                                        'The defualt "True" was used. Jitter options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_meanbars + '"' + ' is not a valid option for meanbars. '
                                          'The defualt "False" was used. Mean bar options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_errorbars + '"' + ' is not a valid option for errorbars. '
                                           'The defualt "None" was used. Error bar options are SD, SEM, or None')
    ...
    try:
        ...
        error_log(
            '"' + config_logscale + '"' + ' is not a valid option for logscale. '
                                          'The defualt "False" was used. Log scale options are True or False.')
    ...
    try:
        ...
        error_log(
            '"' + config_annotation + '"' + ' is not a valid option for annotation. '
                                            'The defualt "False" was used. '
                                            'Significance annotation options are True or False.')

    try:
        config_dpi = config.get('graph_options', 'DPI')
        config_dpi = int(config_dpi)
        assert 300 <= config_dpi <= 600
        config_dict['config_dpi'] = int(config_dpi)
    except (AssertionError, ValueError):
        config_dict['config_dpi'] = int(600)
        error_log('"' + str(
            config_dpi) + '"' + ' is not a valid option for dpi. '
                                'The defualt "600" was used. DPI options are any integer between 300 and 600.')
...
def plot_graph(parsed_data,
    ...
    maxnumbers = [parsedDataFrame.replace('None', np.nan).astype("float").max().max()]
    maxvalue = max(maxnumbers)

    # Get the min for plotting purposes (e.g. don't plot below zero)
    minnumbers = [parsedDataFrame.replace('None', np.nan).astype("float").min().min()]
    ...
    boxColorOrder = []
    if config_boxpalette is not None:
    ...
    dotColorOrder = []
    if config_dotpalette is not None:
    ...
    if file_name_var is 'clonality' or file_name_var is 'tcell':
        ...
        try:
            ...
            if '\\' in graph_name:
                error_log(
                    "Permission Error in plt.savefig(). "
                    "Make sure the metadata headers contain only valid "
                    "characters and that the file is not in use by another program.")
                pass
    ...
    if 'arb' in file_name_var:
        ...
        try:
            ...
            if '\\' in graph_name:
                error_log(
                    "Permission Error in plt.savefig(). "
                    "Make sure the metadata headers contain only valid characters and "
                    "that the file is not in use by another program.")
                pass
...
if __name__ == "__main__":
    ...
    samples_list, metadata_dict, cfg_file, arblist = prep_files(metadatafilename='metadata.tsv',
    ...
    if config_dict['config_overwrite']:
